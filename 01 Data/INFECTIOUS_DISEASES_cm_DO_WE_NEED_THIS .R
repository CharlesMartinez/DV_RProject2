...
require(extrafont)
df_A %>% select(DISEASE, COUNTY, COUNT) %>% filter(DISEASE == "Scombroid Fish Poisoning" | DISEASE == "Paralytic Shellfish Poisoning") %>% ggplot(aes(x = COUNTY, y = COUNT, color = DISEASE)) + ggtitle("Paralytic Shellfish and Scromboid Fish Poisonings in California Counties") + geom_point(position = "jitter") + coord_flip()
